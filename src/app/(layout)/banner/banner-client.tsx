...
interface BannerForm {
  images: FileList;
  images_name: string[];
  title: string[]; // ✅ Thêm trường title vào form
}
...
export default function BannerClient() {
  ...
  const [fileList, setFileList] = useState<File[]>([]);
  const [imageNames, setImageNames] = useState<string[]>([]);
  const [titles, setTitles] = useState<string[]>([]); // ✅ Thêm state để quản lý titles
  const [banner, setBanner] = useState<Banner | null>(null);
  const [submitError, setSubmitError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false);
    ...
    const fetchBanner = async () => {
      try {
        ...
        if (data && data.images?.length > 0) {
          ...
          setFileList(files);
          setImageNames(data.images_name || new Array(files.length).fill(""));
          setTitles(data.title || new Array(files.length).fill("")); // ✅ Khởi tạo titles từ dữ liệu API
        } else {
          setBanner(null);
          setFileList([]);
          setImageNames([]);
          setTitles([]); // ✅ Khởi tạo titles rỗng
        }
      } catch {
        setBanner(null);
        setFileList([]);
        setImageNames([]);
        setTitles([]); // ✅ Khởi tạo titles rỗng khi lỗi
      }
    };
  ...
  const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      setFileList((prev) => [...prev, ...selectedFiles]);
      setImageNames((prev) => [...prev, ...selectedFiles.map(() => "")]);
      setTitles((prev) => [...prev, ...selectedFiles.map(() => "")]); // ✅ Thêm title rỗng cho mỗi ảnh mới
      if (fileInputRef.current) fileInputRef.current.value = "";
    }
  };
  ...
  const removeFile = (index: number) => {
    setFileList((prev) => prev.filter((_, i) => i !== index));
    setImageNames((prev) => prev.filter((_, i) => i !== index));
    setTitles((prev) => prev.filter((_, i) => i !== index)); // ✅ Xóa title tương ứng
    if (fileInputRef.current) fileInputRef.current.value = "";
  };
  ...
  const onSubmit = async () => {
    ...
    formData.append("title", JSON.stringify(titles)); // ✅ Gửi titles dưới dạng JSON string
        ...
        <div className="grid grid-cols-2 gap-4">
          {fileList.map((file, index) => (
            <div key={index} className="relative border rounded p-2">
              ...
              <Input
                type="text"
                value={titles[index] || ""} // ✅ Hiển thị title tương ứng
                onChange={(e) => {
